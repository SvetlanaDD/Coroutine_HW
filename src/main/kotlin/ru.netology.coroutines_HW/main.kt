package ru.netology.coroutines_HW
...
import ru.netology.coroutines_HW.dto.*
                ...
                val posts = getPosts(client)
                    .map { post ->
                        val comments = getComments(client,post.id)
                            .map{ comment ->
                                async { CommentWithAuthor(getAuthor(client,comment.authorId), comment) }
                            }.awaitAll()

                        async {
                            PostWithAuthorAndComments(getAuthor(client,post.authorId), post, comments)
                        }
                    }.awaitAll()
                println(posts)
            } catch (e: Exception) {
